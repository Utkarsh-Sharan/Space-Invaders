...
namespace UI
{
	namespace UIElement
	{
		...
		{
			image_sprite.setTextureRect(texture_rect); //the in-built SFML method setTextureRect()
		}
		...
		void ImageView::setOriginAtCentre()		//sets the 'origin' of the sprite as its center point by dividing its height and width by 2.
		{
			image_sprite.setOrigin(image_sprite.getLocalBounds().width / 2, image_sprite.getLocalBounds().height / 2); 
		}

		void ImageView::setImageAlpha(float alpha)		//alpha describes transparency of an image.
		{
